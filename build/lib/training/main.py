...
class Main:
    def __init__(self, continue_prev=False):
        ...
        if not continue_prev:
            ...
            saved = []
            biggest = get_best_gen(database.get_contents("database"))
    ...
    def get_score(self, individual, opening=0):
        num_games = 20
        tries_limit = 20
        first_game, opening=play_game(individual, tries_limit, opening=opening)
        games=[play_game(individual, tries_limit)[0] for _ in range(num_games-1)]+[first_game]
